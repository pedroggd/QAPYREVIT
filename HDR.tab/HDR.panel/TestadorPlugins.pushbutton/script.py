...
PARAM_DISC_MATCH = {
    ...
    # Mantive as opções originais caso você ainda precise delas em projetos antigos:
    "HI":  ["HIDRAULICA", "PH", "HI"],
    "PH":  ["HIDRAULICA", "PH", "HI"],
    "INC": ["INCENDIO", "PI", "INC"],
    "PI":  ["INCENDIO", "PI", "INC"],
    ...
    with Transaction(doc, "Criar Pranchas e Vistas") as t:
        ...
        for item in pranchas:
            ...
            try:
                ...
                try:
                    # A propriedade Outline retorna o tamanho exato da vista no papel
                    outline = nova_vista.Outline
                    escala_atual = nova_vista.Scale
                    
                    if escala_atual <= 0:
                        escala_atual = 100  # Prevenção contra escala inválida
                        
                    # Multiplica pela escala para converter para a proporção real do modelo
                    modelo_w = abs(outline.Max.U - outline.Min.U) * escala_atual
                    modelo_h = abs(outline.Max.V - outline.Min.V) * escala_atual
                    
                except Exception:
                    # Fallback de segurança caso o Outline falhe
        ...
        with Transaction(doc, "Otimizar Pranchas e Escala") as t:
            ...
            for sheet_id, vp_id in vps_para_centralizar:
                try:
                    ...
                    # Alinha no maximo da esquerda e no maximo do topo
