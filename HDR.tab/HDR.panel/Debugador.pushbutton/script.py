...
__title__ = "SheetFixTESTEMATHEUS+"
...
def escolher_melhor_titleblock(modelo_w, modelo_h, tb_preferido, todos_tbs, nome_vista="", w_leg_max=0, h_leg_total=0):
    ...
    for tb_sym, util_w, util_h, nome_tipo in todos_tbs_ordenados:
        for escala in sorted(ESCALAS_VALIDAS):
            ...
            req_w = vista_w + (w_leg_max + gap_legenda if w_leg_max > 0 else 0)
            req_h = max(vista_h, h_leg_total)
            
            req_w_rot = vista_h + (w_leg_max + gap_legenda if w_leg_max > 0 else 0)
            req_h_rot = max(vista_w, h_leg_total)
            
            cabe_normal   = (req_w <= util_w and req_h <= util_h)
            cabe_rotacion = (req_w_rot <= util_h and req_h_rot <= util_w)
            
            if cabe_normal or cabe_rotacion:
                melhor_tb     = tb_sym
        ...
        for item in pranchas:
            ...
            try:
                ...
                if sb_param and sb_param.AsElementId() != DB.ElementId.InvalidElementId:
                    ...
                    if sb:
                        try:
                            bb_sb = sb.get_BoundingBox(nova_vista)
                ...
                if (modelo_w == 0 or modelo_h == 0) and nova_vista.CropBoxActive:
