...
def executar_fluxo_isometrico():
    ...
    def _limpar_marcacoes(grupos_marc=None):
        ...
        try:
            with DB.Transaction(doc, "Limpar Marcacoes Temporarias") as t_limpa:
                t_limpa.Start()
    ...
    try:
        while True:
            ...
            instrucao = "[%s]  (%d já marcado(s))  —  Desenhe o retângulo ou ESC para opções" % (
            ...
            try:
                ...
                try:
                    with Transaction(doc, "Marcacao: %s" % nome_preview) as t_marc:
                        ...
                        for p1, p2 in [
                            ...
                            dl = doc.Create.NewDetailCurve(
                                vista_ativa, Line.CreateBound(p1, p2)
                            )
                    ...
                    pass  # marcação visual é só feedback; não bloqueia o fluxo
            ...
            # Garante limpeza mesmo em crash inesperado
            _limpar_marcacoes()
    if not grupos:
