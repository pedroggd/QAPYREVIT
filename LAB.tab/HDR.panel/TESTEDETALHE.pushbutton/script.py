...
def executar_fluxo_callout():
    ...
    contador = num_inicial #_proximo_contador(prefixo, identificador, sufixo, zeros, num_inicial)

    caixas_desenhadas = []
    # Cada entrada = lista de ElementIds da marcação daquele callout (índice espelha caixas_desenhadas)
    marcacoes_por_caixa = []

    def _limpar_marcacoes(grupos=None):
        """Deleta marcações temporárias. Se grupos=None, deleta todas."""
        ids_a_deletar = []
        if grupos is None:
    ...
    while True:
        ...
        numero_str_prompt = formato_num.format(contador)
        ...
        try:
            ...
            try:
                with Transaction(doc, "Marcacao: {}".format(nome_preview)) as t_marc:
                    ...
                    for p1, p2 in segmentos:
                        dl = doc.Create.NewDetailCurve(
                            active_view,
                            Line.CreateBound(p1, p2)
                        )
                        ids_marcacao.append(dl.Id)

                    # TextNote centralizado com o nome do detalhe
            ...
            if len(caixas_desenhadas) > 0:
                opcao = forms.alert(
                    "Selecao pausada. Voce possui {} detalhe(s) na fila.\nO que deseja fazer?".format(
                        len(caixas_desenhadas)
                    ),
                ...
                if opcao == "Desfazer o ULTIMO e Continuar":
                    ...
                    contador -= 1
                    # Remove marcação do último callout desenhado
                    if marcacoes_por_caixa:
                        ultimo_grupo = [marcacoes_por_caixa.pop()]
                        _limpar_marcacoes(grupos=ultimo_grupo)
                    continue
