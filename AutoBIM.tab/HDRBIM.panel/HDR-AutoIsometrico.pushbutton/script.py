...
class IsometricConfigWindow(Window):
    ...
    def __init__(self, templates_dict, view_types_dict, titleblock_symbols, viewport_types_dict, legends_dict):
    ...
    def _build_tab_legendas(self):
        ...
        self._lbl(pnl, "Ordem de posicionamento (1a = mais proxima do carimbo):", 11, bold=True, mg=(0, 0, 0, 6))
    ...
    def on_add_legenda(self, sender, args):
        ...
        if nome in self._legendas_ordenadas:
            forms.alert("'%s' ja esta na lista." % nome)
            return
...
def executar_fluxo_isometrico():
    ...
    legends_dict        = get_legend_views()

    dlg = IsometricConfigWindow(templates, view_types_3d, titleblock_symbols, viewport_types_dict, legends_dict)
    dlg.ShowDialog()
    ...
    # -----------------------------------------------------------------------
    # HELPER — inserir legendas na prancha (identico ao script de detalhe)
    # Legendas ficam ACIMA do carimbo, alinhadas à direita, empilhando de
    # baixo pra cima. Titulo do viewport da legenda removido automaticamente.
    # -----------------------------------------------------------------------
    def _inserir_legendas_na_prancha(sh, bb_sh):
        ...
        # Ancora X: borda direita do carimbo
        target_x_max = bb_sh.Max.X - (margem_dir_mm / 304.8)
        # Ancora Y: topo do selo + margem configurada pelo usuario
        ...
        for leg_view in legendas_views:
            ...
            try:
                if not Viewport.CanAddViewToSheet(doc, sh.Id, leg_view.Id):
                    erros.append("Legenda '%s' nao pode ser adicionada." % leg_view.Name)
                    continue
                ...
                # Centro: alinhado a direita, empilhando de baixo pra cima
                ...
                # Remove titulo do viewport da legenda
                try:
                ...
                erros.append("Erro ao inserir legenda '%s': %s" % (leg_view.Name, str(e_leg)))
    ...
    with TransactionGroup(doc, "Gerador de Isometricos") as tg:
        ...
        with Transaction(doc, "Criar Primeira Prancha") as t:
            t.Start()
            try:
                sh, ux_min, ux_max, uy_min, uy_max, max_por_prancha, bb_sh_primeira[0] = nova_prancha()
                _inserir_legendas_na_prancha(sh, bb_sh_primeira[0])
                t.Commit()
            except Exception as e:
                t.RollBack()
                tg.RollBack()
        ...
        for idx_pag in range(1, len(paginas)):
            with Transaction(doc, "AutoIso: Nova Prancha %d" % (idx_pag + 1)) as t_np:
                t_np.Start()
                try:
                    nova_sh, nx_min, nx_max, ny_min, ny_max, max_pp, bb_nova = nova_prancha()
                    _inserir_legendas_na_prancha(nova_sh, bb_nova)
                    t_np.Commit()
